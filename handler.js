...
export async function handler(chatUpdate) {
  ...
  try {
    ...
    if (opts["autoread"]) await this.chatRead(m.key).catch(() => {});
  }
}
...
export async function participantsUpdate({ id, participants, action }) {
  ...
  let gettext = await fetch(
    "https://raw.githubusercontent.com/fawwaz37/random/main/bijak.txt"
  );
  ...
  switch (action) {
    ...
    case "remove":
      if (chat.welcome) {
        const groupMetadata =
          (await this.groupMetadata(id)) || (this.chats[id] || {}).metadata;
        for (let user of participants) {
          const isAddAction = action === "add";
          const welcomeText = isAddAction
            ? (
                chat.sWelcome ||
                this.welcome ||
                conn.welcome ||
                `${emoji.welcome} Selamat datang, @user!`
              )
          ...
          const welthum = "https://telegra.ph/file/432e68f42c68903f023f5.png";
          ...
          const lapor = `\n\n${emoji.mail} *PESAN:* JANGAN LUPA BERDONASI AGAR AUTHOR SEMANGAT UPDATENYA, THANK YOU.`;
          ...
          await this.sendMessage(id, {
            ...
            contextInfo: {
              ...
              externalAdReply: {
                ...
                body: global.author,
                mediaType: 1,
                thumbnailUrl: isAddAction ? welthum : byethum,
    ...
    case "promote":
      const promoteText = (
        ...
        this.spromote ||
        conn.spromote ||
        `${emoji.promote} @user *telah diangkat menjadi Admin*`
    ...
    case "demote":
      const demoteText = (
        ...
        this.sdemote ||
        conn.sdemote ||
        `${emoji.demote} @user *tidak lagi menjadi Admin*`
      ).replace("@user", "@" + participants[0].split("@")[0]);
      if (chat.detect) {
        this.sendMessage(
          id,
...
global.dfail = (type, m, conn) => {
  const userTag = `ʜᴀʟʟᴏ ᴋᴀᴋ *@${m.sender.split("@")[0]}*👋, `;
  const emoji = {
    general: "❌",
    owner: "❌",
    moderator: "❌",
