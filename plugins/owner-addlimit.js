let handler = async (m, { conn, command, text, args }) => {
  ...
  if (command === "addlimit") {
    ...
    if (limitValue > users[user].limit) {
      ...
      conn.reply(
        m.chat,
        `Berhasil mengurangi limit untuk @${
          user.split("@")[0]
        }. Limit kini menjadi 0!`,
        m
      );
    } else {
