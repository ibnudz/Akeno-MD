let handler = async (m, { conn, usedPrefix }) => {
  try {
    ...
    const timeSinceLastAdventure =
      currentTime - global.db.data.users[m.sender].lastadventure;
    ...
    if (healt <= 79) {
      return conn.reply(
        m.chat,
        `⚠️ *Minimal health harus 80* untuk berpetualang. Beli obat dengan ${usedPrefix}shop buy potion <jumlah> dan gunakan dengan ${usedPrefix}use potion <jumlah>. Untuk mendapatkan money dan potion gratis ketik *${usedPrefix}claim*`,
        m
      );
    }
    ...
    if (timeSinceLastAdventure <= timeToWait) {
      return conn.reply(
        m.chat,
        `⌛ *Anda sudah berpetualang*, silahkan menunggu sampai *${timers}* lagi`,
        m
      );
    }
    ...
    const totalHealth =
      health > 60 ? health - kucingHealth - armorHealth : health;
    ...
    const diamond =
      [
        ...
        pickRandom(["0", "1", "2"]),
        pickRandom(["0", "1", "2"]),
        pickRandom(["0", "1", "1", "2", "1", "1", "0"]),
        pickRandom(["0", "0", "1", "2", "2", "1", "1", "0"]),
      ][rubah] || 0;
...
*${rpg.emoticon(
      "healt"
    )} Nyawa berkurang -${health}* karena Kamu berpetualang sampai *${pickRandom(
      [
        "🌏 Ujung dunia",
        "🌌 Luar angkasa",
        "🗺️ Dunia mimpi",
...
*${rpg.emoticon("sampah")} Sampah:* ${sampah}${
      potion === 0 ? "" : `\n*${rpg.emoticon("potion")} Potion:* ${potion}`
    }${iron === 0 ? "" : `\n*${rpg.emoticon("iron")} Iron:* ${iron}`}${
      kayu === 0 ? "" : `\n*${rpg.emoticon("kayu")} Kayu:* ${kayu}`
    }${batu === 0 ? "" : `\n*${rpg.emoticon("batu")} Batu:* ${batu}`}${
      string === 0 ? "" : `\n*${rpg.emoticon("string")} Tali:* ${string}`
    }${
      diamond === 0 ? "" : `\n*${rpg.emoticon("diamond")} Diamond:* ${diamond}`
    }${
      common === 0
        ? ""
        : `\n*${rpg.emoticon("common")} Common Crate:* ${common}`
    }${
      uncommon === 0
        ? ""
    ...
    if (mythic > 0) {
      ...
      conn.reply(
        m.chat,
        `🎉 *Selamat!*\nAnda mendapatkan item *Rare* yaitu *${mythic}* ${rpg.emoticon(
          "mythic"
        )} Mythic Crate`,
        m
      );
    ...
    if (legendary > 0) {
      ...
      conn.reply(
        m.chat,
        `🎉 *Selamat!*\nAnda mendapatkan item *Epic* yaitu *${legendary}* ${rpg.emoticon(
          "legendary"
        )} Legendary Crate`,
        m
      );
