  ...
  if (command === "polisi") {
    ...
    if (subCommand === "cari") {
      ...
      if (Date.now() - player.Waktu_Tertangkap < 30000) {
        return conn.reply(
          m.chat,
          "*⏱️ Anda harus menunggu sebentar sebelum dapat mencari kembali.*",
          m
        );
      }
      ...
      conn.reply(
        m.chat,
        `*🔍 Anda menemukan jejak pencuri level ${level}!* Untuk menangkap pencuri, lakukan tindakan: *${thiefAction.toUpperCase()}*.`,
        m
      );
      ...
      conn.reply(
        m.chat,
        `*👮‍♂️ Status Polisi 👮‍♂️*\n\n🔍 Sedang Mencari Pencuri: ${
          player.State === STATES.SEARCHING ? "Ya" : "Tidak"
        }\n🚓 Pencuri Tertangkap: ${
      ...
      if (args.length === 1) {
        conn.reply(
          ...
          "*🛒 Item Shop 🛒*\n\nKaca Pembesar - 200 coins\n" +
            `Gunakan *${usedPrefix}polisi item kaca-pembesar* untuk membeli kaca pembesar.`,
          m
        );
      } else {
        ...
        if (item === "kaca-pembesar") {
          if (player.Kaca_Pembesar) {
            return conn.reply(
              m.chat,
              "*🛒 Anda sudah memiliki kaca pembesar.*",
              m
            );
          }
          ...
          if (player.Uang < 200) {
            return conn.reply(
              m.chat,
              "*🛒 Uang Anda tidak cukup untuk membeli kaca pembesar.*",
              m
            );
          }
          ...
          conn.reply(
            m.chat,
            "*🛒 Anda berhasil membeli kaca pembesar.* Gunakan 'polisi cari' untuk meningkatkan peluang menangkap pencuri.",
            m
          );
        } else {
      ...
      for (let i = 0; i < leaderboard.length; i++) {
        leaderboardMsg += `${i + 1}. @${leaderboard[i].id.split("@")[0]} - ${
          leaderboard[i].Pencuri_Tertangkap
        } Pencuri Tertangkap\n`;
      }
      ...
      conn.reply(
        m.chat,
        `*👮‍♂️ Sesi permainan Polisi dan Pencuri telah dihentikan.*\n\n${skorMsg}`,
        m
      );
      ...
      if (player.State !== STATES.SEARCHING) {
        return conn.reply(
          m.chat,
          "*🔍 Anda harus mencari pencuri terlebih dahulu dengan perintah 'polisi cari'.*",
          m
        );
      }
      ...
      if (!thiefActions[level].includes(polisiAction)) {
        return conn.reply(
          m.chat,
          `*🚓 Pilihan tindakan Anda (${polisiAction.toUpperCase()}) tidak sesuai dengan hasil yang dicari.*`,
          m
        );
      }
      ...
      if (thiefActions[level].includes(player.ThiefAction)) {
        ...
        conn.reply(
          m.chat,
          `*🚓 Anda berhasil melawan dan menangkap pencuri level ${level}!* Anda mendapatkan imbalan Rp${reward.toLocaleString()}. Total Uang Anda: Rp${player.Uang.toLocaleString()}.`,
          m
        );
      } else {
        conn.reply(
          m.chat,
          "*🚓 Tindakan Anda tidak tepat dan pencuri berhasil lepas!*",
          m
        );
      }
    ...
    conn.reply(
      m.chat,
      "*ℹ️ Gunakan perintah 'polisi' untuk memulai game Polisi dan Pencuri.*",
      m
    );
  }
};
