  ...
  const sections = [
    {
      ...
      rows: [
        ...
        {
          title: "🧬 | OnlyLatinos",
          rowId: `${usedPrefix + command} onlylatinos`,
        },
...
`);
  ...
  m.reply(
    `
✅ *${type}* Se *${isEnable ? "Activó" : "Desactivó"}* ${
      isAll ? "para este bot" : isUser ? "" : "para este chat"
    }
`.trim()
  );
};
handler.help = ["en", "dis"].map((v) => v + "able3 <option>");
handler.tags = ["nable"];
handler.command = /^((en|dis)able3|(tru|fals)e|(turn)?ooo(n|ff)|[01])$/i;
