...
${Object.keys(crateReward)
  .filter(
    (v) =>
      v && crateReward[v] && !/legendary|pet|mythic|diamond|emerald/i.test(v)
  )
  .map((reward) =>
    `
...
`.trim()
  ...
  if (mythic || diamond)
    m.reply(
      `
Congrats you got a rare item, which is ${
...
`.trim()
  ...
  if (pet || legendary || emerald)
    m.reply(
      `
Congrats you got a epic item, which is ${
        pet ? `*${pet}* ${rpg.emoticon("pet")}pet` : ""
      }${
        pet && legendary && emerald
          ? ", "
          : (pet && legendary) || (legendary && emerald) || (emerald && pet)
          ? "and "
          : ""
      }${
        legendary ? `*${legendary}* ${rpg.emoticon("legendary")}legendary` : ""
      }${pet && legendary && emerald ? "and " : ""}${
