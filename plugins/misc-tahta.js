...
function ht(text = "") {
  return new Promise((resolve, reject) => {
    ...
    let yF = `((${pickRandom(["", "-"])}${(45 * w) / 2048}*${pickRandom([
      "sin",
      "cos",
    ])}(X/${w}*4*PI))+${noise("X", 5, w, 0.8)}+${noise("Y", 2, h, 1)})/1.7+128`;
...
function formula(_var, freq, offset, amp, add) {
  return `(${add.toFixed(3)}+${amp.toFixed(4)}*sin(${offset.toFixed(
    6
  )}+2*PI*${_var}*${freq.toFixed(6)}))`;
}
...
function textArgs(
  ...
  return `color=${background}:s=${w}x${h},drawtext=text='${text.replace(
    /[\\]/g,
    "\\$&"
