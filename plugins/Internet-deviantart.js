...
`;
    ...
    if (feature == "info") {
      ...
      try {
        if (
          !inputs.startsWith("https://www.deviantart.com/") &&
          !inputs.startsWith("https://backend.deviantart.com/")
        )
...
async function infoDeviantart(url) {
  const modifiedURL = url.includes("https://backend.deviantart.com/oembed?url=")
    ? url
    : `https://backend.deviantart.com/oembed?url=${url}&format=json`;
  try {
