let handler = async (m, { conn, command, text, args }) => {
  ...
  if (command === "addmoney") {
    ...
    if (moneyValue > users[user].money) {
      ...
      conn.reply(
        m.chat,
        `Berhasil mengurangi money untuk @${
          user.split("@")[0]
        }. Money kini menjadi 0!`,
        m
      );
    } else {
