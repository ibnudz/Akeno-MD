...
var handler = async (m, { conn, command, args, text, usedPrefix }) => {
  if (!text)
    throw `*⚠️ INGRESE EL NOMBRE DE UNA CANCIÓN*\n\n❕ EJEMPLO\n*${
      usedPrefix + command
    }* Another love`;
...
*⌚ ᴛᴜɴɢɢᴜ ᴀᴜᴅɪᴏ sᴇᴅᴀɴɢ ᴅɪ ᴋɪʀɪᴍ...*`.trim();
    ...
    if (command == "play") {
      try {
        ...
        try {
          const dataRE = await fetch(
            `https://api.akuari.my.id/downloader/youtube?link=${yt_play[0].url}`
          );
          ...
          try {
            let humanLol = await fetch(
              `https://api.lolhuman.xyz/api/ytplay?apikey=${lolkeysapi}&query=${yt_play[0].title}`
            );
            ...
            try {
              let lolhuman = await fetch(
                `https://api.lolhuman.xyz/api/ytaudio2?apikey=${lolkey}&url=${yt_play[0].url}`
              );
              ...
              try {
                ...
                let __res = searchh.all
                  .map((v) => v)
                  .filter((v) => v.type == "video");
                let infoo = await ytdl.getInfo(
                  "https://youtu.be/" + __res[0].videoId
                );
                let ress = await ytdl.chooseFormat(infoo.formats, {
                  filter: "audioonly",
                });
                await conn.sendMessage(
                  m.chat,
    ...
    if (command == "play2") {
      try {
        ...
        try {
          ...
          try {
            let lolhuman = await fetch(
              `https://api.lolhuman.xyz/api/ytvideo2?apikey=${lolkey}&url=${yt_play[0].url}`
            );
            let lolh = await lolhuman.json();
            let n = lolh.result.title || "error";
...
function secondString(seconds) {
  ...
  var mDisplay = m > 0 ? m + (m == 1 ? " minutes, " : " menit, ") : "";
    ...
    ytdl
      ...
      .then(async (getUrl) => {
        ...
        let tiny = await axios.get(
          `https://tinyurl.com/api-create.php?url=${resultFix[0].audio}`
        );
  ...
  return new Promise(async (resolve, reject) => {
    ytdl
      ...
      .then(async (getUrl) => {
        ...
        let tiny = await axios.get(
          `https://tinyurl.com/api-create.php?url=${resultFix[0].video}`
        );
        let tinyUrl = tiny.data;
        let title = getUrl.videoDetails.title;
        let thumb =
