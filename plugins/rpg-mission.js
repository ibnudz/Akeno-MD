...
async function handler(m, { conn, args, text, usedPrefix, command }) {
  ...
  try {
    ...
    if (user.health == 0 || user.stamina == 0 || user.mana == 0)
      return m.reply(`your stamina/healt/mana is less than 100`);

    if (typeof user.lastmisi != "number")
      global.db.data.users[m.sender].lastmisi = 0;
    ...
    if (new Date() - user.lastmisi <= cooldown)
      return m.reply(`wait for 🕐${clockString(timers)}`);
