let handler = async (m, { conn, command, args, usedPrefix }) => {
  ...
  try {
    ...
    if (/use|pakai/i.test(command)) {
      let count = /[0-9]/g.test(args[1])
        ? Math.max(args[1] || Math.ceil((100 - healt) / usepotion), 1)
        : Math.ceil((100 - healt) / usepotion);
      ...
      let count = /[0-9]/g.test(args[0])
        ? Math.max(args[0] || Math.ceil((100 - healt) / usepotion), 1)
        : Math.ceil((100 - healt) / usepotion);
