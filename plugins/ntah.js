export async function before(m) {
  ...
  if (chat.autoReply && !isBaileys) {
    if (
      mtype === "groupInviteMessage" ||
      text.startsWith("https://chat") ||
      text.startsWith("Buka tautan ini")
    ) {
      this.reply(
        m.chat,
        `✨ *Undang Bot ke Grup* ✨\n💎 7 Hari / Rp 5,000\n💎 30 Hari / Rp 15,000`,
    ...
    if (mtype === "editedMessage") {
      try {
        ...
        const message_edit = this.loadMessage(m.id).message.editedMessage
          .message.protocolMessage.editedMessage.extendedTextMessage.text;
        const quoted_edit = this.loadMessage(
          this.loadMessage(m.id).message.editedMessage.message.protocolMessage
            .key.id
        );
        return this.sendMessage(
          m.chat,
          { text: `${tittle_edit}\n\n${message_edit}`, mentions: [m.sender] },
          { quoted: quoted_edit }
        );
      } catch (e) {
        console.log(e);
    ...
    if (triggerWords.some((word) => lowerText === word)) {
      // Check if m.text exactly matches any word in the triggerWords array
      const apsih = [
        "Kenapa",
        "Ada apa",
        "Naon meng",
      ...
      const caption = `🤖 *${
        apsih[Math.floor(Math.random() * apsih.length)]
      }* kak @${name} 🗿`;
