    ...
    if (feature == "down") {
      ...
      try {
        ...
        let cap =
          ...
          item.image +
          "\n📝 *Note:* " +
          item.note.replace(/(Note of the author)(.*)/, "$1\n$2") +
          "\n⬇️ *Download:* " +
          item.download;
...
async function searchDafont(q) {
  ...
  const regex =
    /<div class="lv1left dfbg">.*?<span class="highlight">(.*?)<\/span>.*?by <a href="(.*?)">(.*?)<\/a>.*?<\/div>.*?<div class="lv1right dfbg">.*?<a href="(.*?)">(.*?)<\/a>.*?>(.*?)<\/a>.*?<\/div>.*?<div class="lv2right">.*?<span class="light">(.*?)<\/span>.*?<\/div>.*?<div style="background-image:url\((.*?)\)" class="preview">.*?<a href="(.*?)">/g;
  ...
  while ((match = regex.exec(html)) !== null) {
    ...
    const result = {
      ...
      authorLink:
        `https://www.dafont.com/${authorLink.trim()}` || "Tidak diketahui",
      author: author.trim() || "Tidak diketahui",
      themeLink:
        `https://www.dafont.com/${themeLink.trim()}` || "Tidak diketahui",
      theme: theme.trim() || "Tidak diketahui",
      totalDownloads:
        totalDownloads.trim().replace(/[^0-9]/g, "") || "Tidak diketahui",
      previewImage:
        `https://www.dafont.com${previewImage.trim()}` || "Tidak diketahui",
...
async function downloadDafont(link) {
  ...
  const getDownloadLink = () =>
    $("a.dl").attr("href") ? "http:" + $("a.dl").attr("href") : "";
