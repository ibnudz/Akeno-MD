...
export async function before(m, { isAdmin, isBotAdmin }) {
  ...
  if (chat.chatbot) {
    try {
      let inputText = await wetabChat(m.text);
      let regex = /{"data":"([^{}]+)}/g;
      let outputs = inputText
        .match(regex)
      ...
      let api = await fetch(
        "http://api.brainshop.ai/get?bid=153868&key=rcKonOgrUFmn5usX&uid=1&msg=" +
...
async function wetabChat(query) {
  try {
    ...
    const response = await fetch(
      "https://wetabchat.haohuola.com/api/chat/conversation",
      {
        method: "POST",
        headers: {
          "I-App": "hitab",
