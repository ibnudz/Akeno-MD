let handler = async (m, { conn, text, usedPrefix, args, participants }) => {
  ...
  let fkonn = {
    key: {
      ...
      ...(m.chat ? { remoteJid: "6285737235703@s.whatsapp.net" } : {}),
    },
    message: {
      contactMessage: {
  ...
  try {
    ...
    if (e.length)
      await conn
        .reply(
          res,
          `Ada @${anubot} Owner-ku Di Sini, Aku Mau Keluar Aja Dah, Takut Kena Marah.
@${conn.user.jid.split(`@`)[0]} akan keluar 5 detik lagi
Bye😑
Thanks dah invite Gua *${m.name}*`,
          fkonn,
          {
            mentions: d,
          }
        ...
        .then(async () => {
          ...
          await conn.reply(
            ...
            `*INVITING!*\n\n@${m.sender.split("@")[0]} telah mengundang ${
              conn.user.name
            } ke grup\n\n${await conn.getName(res)}\n\n${res}\n\nPesan : ${
              args[0]
            }\n\nBot akan keluar otomatis setelah *${msToDate(
              global.db.data.chats[res].expired - now
            )}*`,
            null,
            { mentions: [m.sender] }
...
*${
            conn.user.name
          }* adalah salah satu Bot WhatsApp Multi-Device yang di bangun dengan Node.js, *${
            conn.user.name
          }* Baru aja di invite oleh *${m.name}*
Untuk menggunakan *${conn.user.name}* silahkan ketik
#menu
@${conn.user.jid.split("@")[0]} akan keluar secara otomatis setelah *${msToDate(
            global.db.data.chats[res].expired - now
          )}*`;
