...
function transformData(data) {
  return data.map(
    ({ currentTitle, currentLink, previousTitle, previousLink }) => ({
      currentTitle: currentTitle.replace(/^\*\d+\.\s*/, "").replace(/\*/g, ""),
      currentLink: currentLink.split("?")[0],
      previousTitle: previousTitle
        .replace(/^(https:\/\/chat\.whatsapp\.com\/[^:]+):?/, "Sebelumnya: ")
        .split("\n")[0],
    ...
    return $(".wa-chat")
      .map((index, element) => ({
        currentTitle: $(element)
          .find(
            ".wa-chat-body .wa-chat-title-container .wa-chat-title .wa-chat-title-text"
          )
          .text()
          .trim(),
        currentLink: $(element)
          .find(".wa-chat-body .wa-chat-title-container a")
          .attr("href"),
        previousTitle: $(element).find(".wa-chat-body .wa-chat-message").text(),
        previousLink: $(element)
          .find(".wa-chat-body .wa-chat-message .URLMessage")
          .attr("href"),
      }))
      .get();
  } catch (error) {
