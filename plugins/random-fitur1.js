...
let handler = async (
  ...
  if (command == "menikah") {
    ...
    while (b === a);
    m.reply(
      `*${toM(a)}, KAU HARUS MENIKAH ${toM(
        b
      )}, kamu MENJADI PASANGAN YANG BAIK 💓*`,
      null,
      {
        mentions: [a, b],
      }
  ...
  if (command == "metercinta") {
    ...
    conn.reply(
      ...
      `
*❤️❤️ METER CINTA ❤️❤️*
*cinta dari ${text} itu untuk kamu* *${Math.floor(
        Math.random() * 100
      )}%* *dari 100%*
...
`;
  ...
  if (command == "beasiswa") {
    let gas = await fetch(
      `https://api.lolhuman.xyz/api/indbeasiswa?apikey=${global.lolkey}`
    );
    ...
    let button = {
      ...
      description: `⚡ ${name} Silakan pilih ${command} di tombol di bawah...\n*Teks yang anda kirim:* ${text}\n\nKetik ulang *${
        usedPrefix + command
      }* teks anda untuk mengubah teks lagi`,
      footerText: wm,
    };
  ...
  if (command == "stimker") {
    ...
    let lis = [
      apilol + "anjing?apikey=" + global.lolkey,
  ...
  if (command == "apkdown") {
    ...
    let f = await fetch(
      `https://api.lolhuman.xyz/api/apkdownloader?apikey=${global.lolkey}&package=${text}`
    );
    let x = await f.json();
...
`;
  ...
  if (command == "proxysite") {
    if (!args[0])
      throw `Contoh penggunaan ${usedPrefix}${command} https://google.com`;

    let f = await fetch(
      `https://api.lolhuman.xyz/api/proxysite?apikey=${global.lolkey}&url=${text}`
    );
    let x = await f.json();
  ...
  if (command == "mirrorcreator") {
    ...
    let f = await fetch(
      `https://api.lolhuman.xyz/api/mirrorcreator?apikey=${global.lolkey}&url=${text}`
    );
...
`;
  ...
  if (command == "ouo") {
    if (!args[0])
      throw `Contoh penggunaan ${usedPrefix}${command} https://ouo.io/8BgQ1w`;

    let f = await fetch(
      `https://api.lolhuman.xyz/api/ouo?apikey=${global.lolkey}&url=${text}`
    );
    let jsons = await f.json();
    let x = jsons.result;
  ...
  if (command == "ouoshort") {
    ...
    let f = await fetch(
      `https://api.lolhuman.xyz/api/ouoshortlink?apikey=${global.lolkey}&url=${text}`
    );
    let jsons = await f.json();
    let x = jsons.result;
    let caption = `*Result:* ${x}`;
  ...
  if (command == "shortlink") {
    ...
    let f = await fetch(
      `https://api.lolhuman.xyz/api/shortlink?apikey=${global.lolkey}&url=${text}`
    );
    let jsons = await f.json();
    let x = jsons.result;
    let caption = `*Result:* ${x}`;
  ...
  if (command == "shortlink2") {
    ...
    let f = await fetch(
      `https://api.lolhuman.xyz/api/shortlink2?apikey=${global.lolkey}&url=${text}`
    );
    let jsons = await f.json();
    let x = jsons.result;
    let caption = `*Result:* ${x}`;
  ...
  if (command == "shortlink3") {
    ...
    let f = await fetch(
      `https://api.lolhuman.xyz/api/shortlink3?apikey=${global.lolkey}&url=${text}`
    );
    let jsons = await f.json();
    let x = jsons.result;
    let caption = `*Result:* ${x}`;
  ...
  if (command == "shortlink4") {
    ...
    let f = await fetch(
      `https://api.lolhuman.xyz/api/shortlink4?apikey=${global.lolkey}&url=${text}`
    );
    let jsons = await f.json();
    let x = jsons.result;
    let caption = `*Result:* ${x}`;
