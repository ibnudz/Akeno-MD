let handler = async (m, { conn, text, groupMetadata }) => {
  ...
  if (!text) {
    var pesan =
      "🚩 Please be active in the group because there will be member cleaning at any time";
  } else {
    var pesan = text;
  }
  var sum;
  ...
  for (let i = 0; i < sum; i++) {
    ...
    if (
      (typeof global.db.data.users[member[i]] == "undefined" ||
        milliseconds * 1 - global.db.data.users[member[i]].lastseen > lama) &&
      !users.isAdmin &&
      !users.isSuperAdmin
    ) {
  ...
  if (total == 0)
    return conn.reply(m.chat, `🚩 *There are no siders in this group.*`, m);
  conn.reply(
    m.chat,
    `*${total}/${sum}* anggota grup *${await conn.getName(
      ...
      .map(
        (v) =>
          ...
          v.replace(
            /@.+/,
            "" + typeof global.db.data.users[v] == "undefined"
              ? " Sider "
              : " Off " +
                  msToDate(milliseconds * 1 - global.db.data.users[v].lastseen)
          )
      )
      .join("\n")}`,
    m,
