...
`.trim()
  ...
  if (user.pickaxe == 0) {
    return m.reply(
      "⛏️ Kamu tidak bisa menambang tanpa alat penambang (pickaxe)! ⛏️"
    );
  }
  ...
  if (new Date() - user.lastmining <= cooldown) {
    return m.reply(
      `
⏳ Kamu sudah menambang sebelumnya! Silakan tunggu *${(timers / 1000).toFixed(
        2
      )} detik* sebelum menambang lagi.
`.trim()
    );
  }
