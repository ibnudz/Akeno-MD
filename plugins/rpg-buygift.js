let handler = async (m, { conn, args, usedPrefix }) => {
  ...
  if (args.length !== 1 || isNaN(args[0])) {
    return conn.reply(
      m.chat,
      `❓ Masukkan total jumlah gift yang ingin kamu beli.\nContoh: *${usedPrefix}buygift 2*`,
      m
    );
  }
  ...
  if (total < 1 || total > maxTotal) {
    return conn.reply(
      m.chat,
      `❌ Maaf, kamu hanya bisa membeli 1 hingga ${maxTotal} gift sekaligus.`,
      m
    );
  }
  ...
  if (user.money < totalPrice) {
    return conn.reply(
      m.chat,
      `❌ Maaf, uangmu tidak cukup untuk membeli ${total} gift.\nTotal biaya: ${totalPrice} Uang\nUang kamu saat ini: ${user.money} Uang`,
      m
    );
  }
  ...
  conn.freegift[m.sender] = {
    ...
    code: conn.freegift[m.sender]?.code
      ? conn.freegift[m.sender].code.concat(giftCodes)
      : giftCodes,
  };
  ...
  conn.reply(
    m.chat,
    `🎁 Kamu telah membeli ${total} gift dengan total ${totalPrice} Uang.\n\nKode Gift mu dengan urutan angka:\n${giftCodes
