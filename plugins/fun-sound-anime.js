...
const handler = async (m, { conn, usedPrefix, text, command }) => {
  ...
  if (args.length === 1 && /^\d+$/.test(args[0])) {
    ...
    if (linkIndex >= 0 && linkIndex < voices.length) {
      ...
      return await m.reply(
        "❌ Indeks link tidak valid. Harap pilih indeks link yang valid.\nContoh penggunaan: *voice 1*"
      );
    }
  }
  ...
  if (args.length === 2 && /^\d+$/.test(args[0]) && /^\d+$/.test(args[1])) {
    ...
    if (linkIndex >= 0 && linkIndex < voices.length) {
      ...
      if (voiceIndex >= 0 && voiceIndex < listVoices.length) {
        ...
        return await m.reply(
          "❌ Indeks suara tidak valid. Harap pilih indeks suara yang valid.\nContoh penggunaan: *voice 1 1*"
        );
      }
    } else {
      return await m.reply(
  ...
  if (args.length === 1 && args[0].toLowerCase() === "v2") {
    ...
    const message =
      voices.length === 0
        ? "❓ Daftar suara kosong."
        : `🔊 *Daftar Suara dari voiceV2():*\n${voices
            .map((voice, index) => `*${index + 1}.* ${voice.text}`)
            .join("\n")}`;
    return await m.reply(message);
  }
  ...
  if (args[0].toLowerCase() === "v3" && args[1]) {
    const voiceV3Sounds = {
      ...
      ganbare:
        "https://andgyk.is-a.dev/anime-soundboard/audio/ganbare-ganbare-senpai.mp3",
      konichiwa:
        "https://andgyk.is-a.dev/anime-soundboard/audio/hashira-konichiwa.mp3",
      ...
      ultra:
        "https://andgyk.is-a.dev/anime-soundboard/audio/ultra-instinct.mp3",
      ahh: "https://andgyk.is-a.dev/anime-soundboard/audio/yemete-kudasai-ah.mp3",
      yemete:
        "https://andgyk.is-a.dev/anime-soundboard/audio/yemete-kudasai.mp3",
...
async function voiceV1() {
  try {
    ...
    return $("div.column.two-third.column_column table tbody tr")
      ...
      .get()
      .filter(({ name, link }) => name !== "" && link !== undefined);
  } catch (error) {
