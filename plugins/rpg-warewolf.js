...
let handler = async (m, { conn, command, usedPrefix, text }) => {
  ...
  if (inputcmd === "start") {
    ...
    if (players.length < 2)
      return m.reply(
        "😅 Jumlah pemain belum mencukupi. Minimal 2 pemain diperlukan untuk memulai permainan."
      );
    if (players.length > maxPlayers)
      return m.reply("😅 Jumlah pemain melebihi batas maksimal (8 pemain).");

    conn.warewolf[groupId].started = true;
    m.reply("Permainan sudah dimulai. Selamat bermain!");
  } else if (inputcmd === "join") {
    if (started)
      return m.reply("😅 Permainan sudah dimulai, tidak bisa bergabung lagi.");
    if (players.length >= maxPlayers)
      return m.reply(
        `😅 Jumlah pemain sudah mencapai batas maksimal (${maxPlayers} pemain). Permainan bisa dimulai dengan mengetik *${usedPrefix}warewolf start*`
      );
    ...
    if (players.length === 0)
      return m.reply("😅 Belum ada pemain yang bergabung dalam permainan.");

    const warewolf = findWarewolf(players);
    m.reply(
      `🐺 Siapakah Warewolf dalam permainan ini?\n\nPilih salah satu angka:\n${players
    ...
    if (playerIndex < 0 || playerIndex >= players.length)
      return m.reply("😅 Pilih player yang ada saja.");

    const warewolf = findWarewolf(players);
    if (
      playerIndex === players.findIndex((player) => player.role === "warewolf")
    ) {
      m.reply(
        `🎉 Selamat! ${warewolf.name} adalah Warewolf! Permainan berakhir.`
      );
