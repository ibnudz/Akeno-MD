...
async function handler(m, { conn, text }) {
  ...
  if (m.sender in conn.tambang) {
    if (
      conn.tambang[m.sender].currentArea >= conn.tambang[m.sender].areas.length
    ) {
      return m.reply("🏆 Anda telah menyelesaikan semua area pertambangan.");
    }
    return m.reply(
      "⏳ Anda masih memiliki area pertambangan yang belum selesai. Silakan selesaikan terlebih dahulu."
    );
  } else {
    if (text === "start") {
      ...
      if (timers > 0)
        return m.reply(
          `Silakan tunggu ${formatTime(
            timers
          )} lagi sebelum memulai pertambangan baru.`
        );
      ...
      instructions +=
        "Ketik *'stop'* untuk menghentikan pertambangan saat sedang bermain.";
...
handler.before = async (m) => {
  ...
  let { areas, currentArea, hasilTambang, lastTambangTime, totalReward } =
    conn.tambang[m.sender];
  const cooldown = 5 * 60 * 1000; // 5 menit cooldown
  let user = global.db.data.users[m.sender];
  ...
  if (msg === "stop") {
    m.reply(
      "❌ Pertambangan telah dihentikan. Ketik *'tambang start'* untuk memulai pertambangan kembali."
    );
