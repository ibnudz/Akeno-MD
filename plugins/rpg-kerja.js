...
let handler = async (m, { conn, command, args, usedPrefix }) => {
  ...
  if (/kerja|work/i.test(command)) {
    const jobFields = Object.keys(jobs)
      .map((field, index) => `*${index + 1}.* ${field}`)
      .join("\n");
    if (!type)
      throw `ℹ️ Pilih bidang pekerjaan yang sesuai:\n${jobFields}\n\nContoh: ketik *${
        usedPrefix + command
      } petani* untuk bekerja pertanian`;

    let jobData = jobs[type]?.[Math.floor(Math.random() * jobs[type]?.length)];
    if (!jobData)
      throw (
        "😅 Pekerjaan tidak ditemukan. Silakan pilih bidang pekerjaan yang sesuai dari daftar berikut:\n" +
        jobFields +
    ...
    const randomMessage = pickRandom([
      `👷 Kamu ${jobData.name} dan sedang ${jobData.task}\nTingkat Kesulitan: ${
        jobData.difficulty
      }\n\n💰 Mendapatkan uang senilai *${formatRupiah(
        earnedMoney
      )}*\n🔼 Dapatkan *${earnedExp}* EXP\nℹ️ Info Tambahan: ${taskInfo}`,
      `🔧 Sebagai ${jobData.name}, tugasmu adalah ${
        jobData.task
      }\nTingkat Kesulitan: ${
        jobData.difficulty
      }\n\n💰 Mendapatkan uang senilai *${formatRupiah(
      ...
      `🚜 Sebagai seorang ${jobData.name}, tugasmu adalah ${
        jobData.task
      }\nTingkat Kesulitan: ${
        jobData.difficulty
      }\n\n💰 Mendapatkan uang senilai *${formatRupiah(
    ...
    setTimeout(() => {
      m.reply(
        "Waktumu untuk bekerja sudah tiba! Kamu bisa bekerja lagi sekarang."
      );
    }, cooldown);
  }
};
