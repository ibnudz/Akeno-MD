    ...
    if (feature == "v4") {
      ...
      try {
        ...
        const buffer = await video.download({
          progress: (p) => {
            console.log(
              `Downloaded ${p.progress}% (${p.downloaded}/${p.total} bytes)`
            );
          },
        });
...
function getEmojiCount(count) {
  const emojis = ["👍", "❤️", "🔁", "💬", "🔥"];
  return (
    emojis[Math.floor(Math.random() * emojis.length)] + count.toLocaleString()
  );
}
