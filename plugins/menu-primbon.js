...
let handler = async (m, { conn, args, text, usedPrefix, command }) => {
  switch (command) {
    ...
    case "nomorhoki":
      {
        if (!Number(text))
          return m.reply(`Contoh : ${usedPrefix + command} 6288292024190`);
    ...
    case "artinama":
      {
        if (!text)
          return m.reply(`Contoh : ${usedPrefix + command} Dika Ardianta`);
    ...
    case "cocoknama":
      {
        if (!text)
          return m.reply(`Contoh : ${usedPrefix + command} Dika, 7, 7, 2005`);
    ...
    case "pasangan":
      {
        if (!text)
          return m.reply(`Contoh : ${usedPrefix + command} Dika|Novia`);
    ...
    case "jadiannikah":
      {
        if (!text)
          return m.reply(`Contoh : ${usedPrefix + command} 6, 12, 2020`);
    ...
    case "sifatusaha":
      {
        if (!ext)
          return m.reply(`Contoh : ${usedPrefix + command} 28, 12, 2021`);
        let [tgl, bln, thn] = text.split`,`;
    ...
    case "rezeki":
      {
        if (!text)
          return m.reply(`Contoh : ${usedPrefix + command} 7, 7, 2005`);
        let [tgl, bln, thn] = text.split`,`;
    ...
    case "fengshui":
      {
        if (!text)
          return `Contoh : ${usedPrefix + command} Dika, 1, 2005\n\nNote : ${
            usedPrefix + command
    ...
    case "memancing":
      {
        if (!text)
          return m.reply(`Contoh : ${usedPrefix + command} 12, 1, 2022`);
        let [tgl, bln, thn] = text.split`,`;
    ...
    case "masasubur":
      {
        if (!text)
          return m.reply(
            `Contoh : ${usedPrefix + command} 12, 1, 2022, 28\n\nNote : ${
              usedPrefix + command
