    ...
    if (/cekapi(key)?|cekkey/i.test(command)) {
      ...
      switch (type) {
        case "xteam":
          if (type == "xteam") {
            let xteam = await (
              await fetch(`https://api.xteam.xyz/cekey?APIKEY=${args[0]}`)
            )
              .json()
              .catch((v) => "error");
            await m.reply(cek);
            if (xteam == "error") {
              ...
              if (xteam.response == "Only alphanumeric!")
                return m.reply("Only alphanumeric!");
              if (
                xteam.response ==
                "Apikey tidak ditemukan, silahkan daftar atau beli ke developer untuk mendapatkan apikey yang valid!"
              )
                return m.reply("ɪɴᴠᴀʟɪᴅ ᴀᴘɪᴋᴇʏ !");
              conn.reply(
                m.chat,
                `• *ᴛʏᴘᴇ:* XTEAM
• *ᴀᴘɪᴋᴇʏ:* ${args[0]}
...
• *ᴇxᴘɪʀᴇᴅ:* ${xteam.response.expired.replace("Premium left:", "")}`,
                m
              );
            }
          }
        ...
        case "lolhuman":
          let lol = await (
            await fetch(
              `https://api.lolhuman.xyz/api/checkapikey?apikey=${args[0]}`
            )
          ).json();
          m.reply(cek);
...
• *Result:* ${oexec}`,
        ...
        default:
          return conn.sendButton(
            m.chat,
            `*${htki} CEK APIKEY ${htka}*`,
            "sᴇʟᴇᴄᴛ ᴛʏᴘᴇ ᴀᴘɪᴋᴇʏ ʜᴇʀᴇ!",
            null,
            [
