let handler = async (m, { conn }) => {
  ...
  if (/template/.test(m.quoted.mtype))
    caption =
      m.quoted.mediaMessage[Object.keys(m.quoted.mediaMessage)[0]].caption;
  else if (/product/.test(m.quoted.mtype))
    caption = m.quoted.product.title + "\n" + m.quoted.product.description;
