...
let handler = async (m, { conn, args, command, text, usedPrefix }) => {
  ...
  listData.forEach(({ id, name }) => listMessage.push(`${id}. ${name}`));
  listMessage.push("=====================");
  ...
  if (!input) {
    ...
    const pesan =
      "Pesan: Angka tidak boleh lebih dari 5!\n" + listMessage.join("\n");
    return m.reply(pesan);
  } else {
    ...
    const data = Object.entries(global.db.data.users).sort(
      (a, b) => b[1].money - a[1].money
    );
    const transformedData = data
      .slice(0, 10)
      .map(([jid, { money, name }], index) => ({
        top: index + 1,
        avatar: "https://cdn.discordapp.com/embed/avatars/0.png",
        tag: name,
        score: money,
      }));
    let dataArray = Object.keys(global.db.data.users).sort(
      (a, b) => b[1].money - a[1].money
    );
    const transformedDataResult = await Promise.all(
      transformedData.map(async (item, index) => ({
        ...item,
        avatar: await conn
          .profilePictureUrl(dataArray[index], "image")
          .catch((_) => logo),
      }))
    );
    ...
    if (args[0] == "1") {
      ...
      let sortedpotion = Object.entries(global.db.data.users).sort(
        (a, b) => b[1].potion - a[1].potion
      );
      let sortedsampah = Object.entries(global.db.data.users).sort(
        (a, b) => b[1].sampah - a[1].sampah
      );
      let sortedmakananpet = Object.entries(global.db.data.users).sort(
        (a, b) => b[1].makananpet - a[1].makananpet
      );
      let sortedbatu = Object.entries(global.db.data.users).sort(
        (a, b) => b[1].batu - a[1].batu
      );
      let sortediron = Object.entries(global.db.data.users).sort(
        (a, b) => b[1].iron - a[1].iron
      );
      let sortedkayu = Object.entries(global.db.data.users).sort(
        (a, b) => b[1].kayu - a[1].kayu
      );
      let sortedstring = Object.entries(global.db.data.users).sort(
        (a, b) => b[1].string - a[1].string
      );
      let sortedcommon = Object.entries(global.db.data.users).sort(
        (a, b) => b[1].common - a[1].common
      );
      let sorteduncommon = Object.entries(global.db.data.users).sort(
        (a, b) => b[1].uncommon - a[1].uncommon
      );
      let sortedmythic = Object.entries(global.db.data.users).sort(
        (a, b) => b[1].mythic - a[1].mythic
      );
      let sortedlegendary = Object.entries(global.db.data.users).sort(
        (a, b) => b[1].legendary - a[1].legendary
      );
      let sortedpet = Object.entries(global.db.data.users).sort(
        (a, b) => b[1].pet - a[1].pet
      );
...
╰┫🐈Kucing ${
        kucing == 0
          ? "Tidak Punya"
          : "" || (kucing > 0 && kucing < 5)
          ? `Level *${kucing}* To level *${
              kucing + 1
            }*\n╭┫Exp *${_kucing}* -> *${kucing * 100}*`
          : "" || kucing == 5
          ? "*Max Level*"
          : ""
      }
...
╰┫🐶Anjing ${
        anjing == 0
          ? "Tidak Punya"
          : "" || (anjing > 0 && anjing < 5)
          ? `Level *${anjing}* To level *${
              anjing + 1
            }*\n╭┫Exp *${_anjing}* -> *${anjing * 100}*`
          : "" || anjing == 5
          ? "*Max Level*"
          : ""
      }
...
3.Top Diamond *${usersdiamond.indexOf(m.sender) + 1}* dari *${
        usersdiamond.length
      }*
...
6.Top Makanan Pet *${usersmakananpet.indexOf(m.sender) + 1}* dari *${
        usersmakananpet.length
      }*
...
10.Top String *${usersstring.indexOf(m.sender) + 1}* dari *${
        usersstring.length
      }*
11.Top Common *${userscommon.indexOf(m.sender) + 1}* dari *${
        userscommon.length
      }*
13.Top Uncommon *${usersuncommon.indexOf(m.sender) + 1}* dari *${
        usersuncommon.length
      }*
14.Top Mythic *${usersmythic.indexOf(m.sender) + 1}* dari *${
        usersmythic.length
      }*
15.Top Legendary *${userslegendary.indexOf(m.sender) + 1}* dari *${
        userslegendary.length
      }*
...
`.trim();
      ...
      let sortedarlok = Object.entries(global.db.data.users).sort(
        (a, b) => b[1].arlok - a[1].arlok
      );
      let sortedlevel = Object.entries(global.db.data.users).sort(
        (a, b) => b[1].level - a[1].level
      );
      ...
      let number = `${PhoneNumber(
        "+" + pasangan.replace("@s.whatsapp.net", "")
...
╰┫🐺 *Serigala :* ${
        serigala == 0
          ? "❌"
          : "" || (serigala > 0 && serigala < 5)
          ? `Level *${serigala}* ➠ *${
              serigala + 1
            }*\n╭┫Exp *${_serigala}* -> *${serigala * 100}*`
          : "" || serigala == 5
          ? "*Max Level*"
          : ""
      }
...
│💎 *Top Diamond:* ${usersdiamond.indexOf(m.sender) + 1} / ${
        usersdiamond.length
      }
...
│🛍️ *Top Uncommon:* ${usersuncommon.indexOf(m.sender) + 1} / ${
        usersuncommon.length
      }
│🎁 *Top Mythic:* ${usersmythic.indexOf(m.sender) + 1} / ${usersmythic.length}
│🧰 *Top Legendary:* ${userslegendary.indexOf(m.sender) + 1} / ${
        userslegendary.length
      }
...
╰┫Phonix ${
        phonix == 0
          ? "Tidak Punya"
          : "" || (phonix > 0 && phonix < 15)
          ? `Level *${phonix}* To level *${
              phonix + 1
            }*\n╭┫Exp *${_phonix}* -> *${phonix * 10000}*`
          : "" || phonix == 15
          ? "*Max Level*"
          : ""
      }
...
╰┫Centaur ${
        centaur == 0
          ? "Tidak Punya"
          : "" || (centaur > 0 && centaur < 20)
          ? `Level *${centaur}* To level *${
              centaur + 1
            }*\n╭┫Exp *${_centaur}* -> *${centaur * 10000}*`
          : "" || centaur == 20
          ? "*Max Level*"
          : ""
      }
...
╰┫Griffin ${
        griffin == 0
          ? "Tidak Punya"
          : "" || (griffin > 0 && griffin < 15)
          ? `Level *${griffin}* To level *${
              griffin + 1
            }*\n╭┫Exp *${_griffin}* -> *${griffin * 10000}*`
          : "" || griffin == 15
          ? "*Max Level*"
          : ""
      }
...
╰┫Serigala ${
        serigala == 0
          ? "Tidak Punya"
          : "" || (serigala > 0 && serigala < 15)
          ? `Level *${serigala}* To level *${
              serigala + 1
            }*\n╭┫Exp *${_serigala}* -> *${serigala * 10000}*`
          : "" || serigala == 15
          ? "*Max Level*"
          : ""
      }
...
6.Top Uncommon *${usersuncommon.indexOf(m.sender) + 1}* dari *${
        usersuncommon.length
      }*
7.Top Mythic *${usersmythic.indexOf(m.sender) + 1}* dari *${usersmythic.length}*
8.Top Legendary *${userslegendary.indexOf(m.sender) + 1}* dari *${
        userslegendary.length
      }*
...
╰┫pancingan ${
        pancingan == 0
          ? "Tidak Punya"
          : "" || (pancingan > 0 && pancingan < 5)
          ? `Level *${pancingan}* To level *${
              pancingan + 1
            }*\n╭┫Exp *${_pancingan}* -> *${pancingan * 10000}*`
          : "" || pancingan == 5
          ? "*Max Level*"
          : ""
      }
