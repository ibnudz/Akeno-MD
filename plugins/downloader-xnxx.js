...
let handler = async (m, { conn, args, text, usedPrefix, command }) => {
  ...
  if (!chat.nsfw)
    throw `🚫 Grup ini tidak mengizinkan konten nsfw \n\nUntuk mengaktifkannya, ketik \n*${usedPrefix}enable* nsfw`;
  let user = global.db.data.users[m.sender].age;
  if (user < 17)
    throw `❎ Kamu belum cukup umur! Silakan kembali ketika kamu berusia di atas 18 tahun`;
  if (!text)
    throw `✳️ Untuk mencari\n📌 Gunakan: *${
      usedPrefix + command
  ...
  if (text.includes("http://") || text.includes("https://")) {
    if (!text.includes("xnxx.com"))
      return m.reply(`❎ Masukkan link dari *xnxx.com*`);
    try {
      let xn = await fg.xnxxdl(text);
      conn.sendFile(
        m.chat,
        xn.result.files.high,
        xn.result.title + ".mp4",
        `
≡  *XNXX DL*
