...
let handler = async (m, { conn }) => {
  ...
  try {
    ...
    conn.reply(
      m.chat,
      `Berikut adalah informasi monitor:\n\n${monitorData}\nTotal Monitor Aktif: ${account.up_monitors}\nTotal Monitor Tidak Aktif: ${account.down_monitors}\nTotal Monitor Dinonaktifkan: ${account.paused_monitors}`,
      m
    );
  } catch (error) {
    m.reply(
      "Maaf, terjadi kesalahan. Kemungkinan API key telah mencapai batas limit (10 req/min). Harap tunggu selama 1 menit untuk melakukan permintaan lagi."
    );
    console.error(error);
  }
};
