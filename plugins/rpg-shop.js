...
${Object.keys(listItems)
                    .map((v) => {
                        ...
                        return `👉 1 ${global.rpg.emoticon(v)}${v} ﹫ ${listItems[v][paymentMethod]
...
${Object.keys(listItems)
                    ...
                    .join("\n")}
`.trim();
    image = command.toLowerCase() == "buy" ? imgr + "buy" : imgr + "sell";
    buttons =
        command.toLowerCase() == "buy"
            ? [
    ...
    if (!listItems[item])
        return conn.sendFile(m.chat, image, "", text + "\n" + footer, m);
    if (command.toLowerCase() == "buy") {
        ...
        if (user[paymentMethod] < listItems[item][paymentMethod] * total)
            return conn.sendFile(
                ...
                imgr + "lowcredit",
                "",
                `*–『 INSUFFICIENT CREDIT 』–*\nʏᴏᴜ ɴᴇᴇᴅ ᴇxᴛʀᴀ *${listItems[item][paymentMethod] * total - user[paymentMethod]
                    }* ${global.rpg.emoticon(
                        paymentMethod
                    )}${paymentMethod} ᴛᴏ ʙᴜʏ *${total}* ${global.rpg.emoticon(
                        item
                    )}${item}.
ʏᴏᴜ'ᴠᴇ *${user[paymentMethod]}* ${global.rpg.emoticon(
                        paymentMethod
                    )}${paymentMethod} ɪɴ ʙᴀɢ.
–––––––––––––––––––––––––
💁🏻‍♂ ᴛɪᴩ :
...
`.trim(),
        ...
        return conn.sendFile(
            ...
            imgr + "bought",
            "",
            `*${htki} BOUGHT ${htka}*\nʏᴏᴜ *ʙᴏᴜɢʜᴛ ${total} ${global.rpg.emoticon(
                item
            )}${item}*.
`.trim(),
            m
        ...
        if (user[item] < total)
            return m.reply(
                `You don't have enough *${global.rpg.emoticon(
                    item
                )}${item}* to sell, you only have ${user[item]} items`
            );
        user[item] -= total;
        ...
        return conn.sendFile(
            ...
            imgr + "sold",
            "",
            `*${htki} SOLD ${htka}*\nʏᴏᴜ *sᴏʟᴅ ${total} ${global.rpg.emoticon(
                item
            )}${item}*.
`.trim(),
            m
