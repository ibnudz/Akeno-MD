...
${soundAlphabets.map((item) => `• ${item}`).join("\n")}`;
    ...
    if (isNumber(text)) {
      try {
        vn =
          "https://raw.githubusercontent.com/AyGemuy/Sound/main/sound" +
          text +
          ".mp3";
      } catch {
...
${usedPrefix + command} 1`;
    ...
    try {
      vn =
        "https://raw.githubusercontent.com/AyGemuy/Rest-Sound/main/HyuuraKane/mangkane" +
        text +
        ".mp3";
    } catch {
      vn =
        "raw.githubusercontent.com/WH-MODS-BOT/Soundskane/master/mangkane" +
        text +
        ".mp3";
    }
    ...
    if (args[0] > 25) {
      const ya =
        "https://raw.githubusercontent.com/AyGemuy/mangkane/main/Mangkanenya/mangkane" +
        args[0] +
        ".mp3";
...
${usedPrefix + command} black cover`;
    ...
    const vn = await fetch(
      global.API("btchx", "/api/search/ringtone", { text: text }, "apikey")
    );
    const dapet = await vn.json();
    const listSections = Object.values(dapet.result).map(
      (v, index) =>
        `${index} ${cmenub} ${v.title}\nAudio 🎧 ${usedPrefix + command}get ${
          v.audio
        }\n*Source:* ${v.source}`
    );
    conn.sendMessage(
      ...
      "conversation",
      {
        contextInfo: { mentionedJid: conn.parseMention(listSections) },
        quoted: m,
      }
    );
