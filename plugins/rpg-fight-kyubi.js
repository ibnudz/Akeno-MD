let handler = async (m, { conn, usedPrefix, participants }) => {
  ...
  if (
    typeof conn.fightnaga[m.sender] != "undefined" &&
  ...
  while (
    typeof global.db.data.users[lawan] == "undefined" ||
    lawan == m.sender
  ) {
    lawan = users[Math.floor(users.length * Math.random())];
  }
  ...
  m.reply(
    `*Pet Kamu* (🦊kyubi ${
      global.db.data.users[m.sender].kyubi
    }) ⚔️menantang 🦊kyubinya *${conn.getName(lawan)}* (🦊kyubi ${
      global.db.data.users[lawan].kyubi
  ...
  let i;
  let unggul;
  for (i = 0; i < global.db.data.users[m.sender].kyubi; i++)
    kesempatan.push(m.sender);
  for (i = 0; i < global.db.data.users[lawan].kyubi; i++)
  ...
  if (pointPemain > pointLawan) {
    ...
    m.reply(
      `*${conn.getName(m.sender)}* [${pointPemain * 10}] - [${
        ...
        global.db.data.users[m.sender].kyubi
      }) MENANG melawan 🦊kyubinya *${conn.getName(lawan)}* (kyubi ${
        global.db.data.users[lawan].kyubi
      }) karena kyubi🦊kamu ${
        alasanMenang[Acakin(0, alasanMenang.length - 1)]
    ...
    m.reply(
      `*${conn.getName(m.sender)}* [${pointPemain * 10}] - [${
        ...
        global.db.data.users[m.sender].kyubi
      }) KALAH melawan 🦊kyubinya *${conn.getName(lawan)}* (kyubi ${
        global.db.data.users[lawan].kyubi
      }) karena pet kamu ${
        alasanKalah[Acakin(0, alasanKalah.length - 1)]
