...
let handler = async (m, { conn, text, usedPrefix: _p }) => {
  ...
  let res = (
    await axios.get(API("can", "/api/anime/nekopoi/detail", { id: text }))
  ).data;
  try {
    let v = res.stream;
    ...
    let arr = [];
    let tekss =
      res.episode && Array.isArray(res.episode)
        ? res.episode
            .map(
              (v) =>
                `${v.title}\nUpload: ${v.date || "Tidak diketahui"} || ID: ${
                  v.id
                }`
            )
            .filter((v) => v)
