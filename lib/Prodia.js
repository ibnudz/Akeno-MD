...
export const getModels = async (select) => {
  ...
  const regex =
    /<script defer="defer" src="(\/js\/app\.[a-f\d]+\.js)"><\/script>/;
  const match = response.data.match(regex);
  const jsPath = match[1];
  const jsResponse = await axios.get(`${app_base}${jsPath}`);
  const modelsMatch = jsResponse.data.match(
    /VUE_APP_AI_MODELS:'(.*?)',VUE_APP_STATS_STREAMS/
  );
...
export const draw = async (options) => {
  const { modelIds, model, quantity = 1, comp } = options;
  if (!model || model == "auto") {
    options.model = modelIds.find((modelId) =>
      modelId.startsWith("anything-v4.5")
    );
  }
  const images = [];
