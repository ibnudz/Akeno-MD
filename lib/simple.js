...
PHOTO;BASE64:${(
                  (await conn
                    .getFile(await conn.profilePictureUrl(njid))
                    .catch((_) => ({}))) || {}
                ).number?.toString("base64")}
    ...
    sendKontak: {
      async value(jid, data, quoted, options) {
        ...
        for (let [number, nama, ponsel, email] of data) {
          number = number.replace(/[^0-9]/g, "");
          let njid = number + "@s.whatsapp.net";
          let name = global.db.data.users[njid]
            ? global.db.data.users[njid].name
            : conn.getName(njid);
          let biz =
            (await conn.getBusinessProfile(njid).catch((_) => null)) || {};
          let vcard = `
...
async function generateProfilePicture(mediaUpload) {
  ...
  const { read, MIME_JPEG, AUTO } = await Promise.resolve().then(
    async () => (await import("jimp")).default
  );
