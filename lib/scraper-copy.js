  ...
  return new Promise((resolve, reject) => {
    fetch(
      ...
      .then((res) => {
        ...
        data.splice(2, 1);
        if (data.length == 0)
          return resolve({ creator: "stikerin", status: false });
...
function doujindesusearch(query) {
  return new Promise((resolve, reject) => {
    axios
      .get(`https://212.32.226.234/?s=${query}`)
      .then(({ data }) => {
        const $ = cheerio.load(data);
        const hasil = [];
...
function playstore(query) {
  return new Promise((resolve, reject) => {
    axios
      .get(`https://play.google.com/store/search?q=${query}&c=apps`)
      .then(({ data }) => {
        const $ = cheerio.load(data);
        const hasil = [];
        $(
          "#yDmH0d > c-wiz.SSPGKf.glB9Ve > div > div > c-wiz > c-wiz > c-wiz > section > div > div > div > div"
        ).each(function (a, b) {
          result = {
            status: 200,
            author: "@lui",
            video_preview: $(b)
              .find("div > div > div > a > div.Vc0mnc > div > button")
              .text(),
...
function sektekomiksearch(query) {
  return new Promise((resolve, reject) => {
    axios
      .get(`https://sektekomik.xyz/manga?search=${query}`)
      .then(({ data }) => {
        const $ = cheerio.load(data);
        const hasil = [];
        $("#swiper-wrapper-32f18f5ed9677ea4 > div").each(function (a, b) {
          result = {
            ...
            thumb: $(b)
              .find("> div > div.product__item__pic.set-bg.manga")
              .attr("data-setbg"),
            type: $(b)
              .find(
                "> div > div.product__item__pic.set-bg.manga > div.ep.m-type > a"
              )
              .text(),
          };
          hasil.push(result);
...
function nekopoisearch(query) {
  return new Promise((resolve, reject) => {
    axios
      .get(`https://nekopoi.care/search/${query}`)
      .then(({ data }) => {
        const $ = cheerio.load(data);
        const hasil = [];
        $("#content > div.postsbody > div.result > ul > li").each(function (
          a,
          b
        ) {
          result = {
...
function kusonimelatest() {
  return new Promise((resolve, reject) => {
    axios
      .get(`https://kusonime.com`)
      .then(({ data }) => {
        const $ = cheerio.load(data);
        const hasil = [];
        $(
          "#venkonten > div.vezone > div.venser > div > div.rseries > div.rapi > div.venz > ul > div"
        ).each(function (a, b) {
          result = {
      ...
      .then(({ data }) => {
        ...
        $("#boxid > div").each(function (a, b) {
          result = {
            ...
            url: $(b)
              .find("> div.eroinfo > span:nth-child(3) > a")
              .attr("href"),
          };
          hasil.push(result);
        });
...
function otakudesu(query) {
  return new Promise((resolve, reject) => {
    axios
      .get(`https://otakudesu.video=/?s=${query}&post_type=anime`)
      .then(({ data }) => {
        const hasil = [];
        const $ = cheerio.load(data);
...
function doujindesu(url) {
  return new Promise((resolve, reject) => {
    ...
    axios.get(url).then((res) => {
      ...
      hasil.status = $(
        "#archive > div > section > table > tbody > tr:nth-child(1) > td:nth-child(2) > a"
      ).text();
      hasil.type = $(
        "#archive > div > section > table > tbody > tr.magazines > td:nth-child(2) > a"
      ).text();
      hasil.series = $(
        "#archive > div > section > table > tbody > tr.parodies > td:nth-child(2) > a"
      ).text();
      hasil.author = $(
        "#archive > div > section > table > tbody > tr:nth-child(4) > td:nth-child(2) > a"
      ).text();
      hasil.group = $(
        "#archive > div > section > table > tbody > tr:nth-child(5) > td:nth-child(2) > a"
      ).text();
      hasil.rating = $(
        "#archive > div > section > table > tbody > tr:nth-child(6) > td:nth-child(2) > div"
      ).text();
      hasil.upload = $(
        "#archive > div > section > table > tbody > tr.created.createdAt > td:nth-child(2)"
      ).text();
      hasil.synopsis =
        $("#archive > div > section > div.pb-2 > p:nth-child(1)").text() ||
        "tidak ada sinopsis karena bukan manhwa!!";
      hasil.chapter = [];
    });
      ...
      $("#chapter_list > ul > li").each(function (a, b) {
        result = {
          ...
          url:
            ...
            $(b).find("> div.epsright > span > a").attr("href"),
          dl_url:
            $(b).find("> div.chright > span > a").attr("href") ||
            "Eror link mungkin telah di hapus oleh admin webnya",
        };
...
function kusonimeinfo(url) {
  return new Promise((resolve, reject) => {
    axios.get(url).then((res) => {
      ...
      const hasil = {};
      hasil.title = $(
        "#venkonten > div.vezone > div.venser > div.post-thumb > h1"
      ).text();
      hasil.info = $(
        "#venkonten > div.vezone > div.venser > div.venutama > div.lexot > div.info"
      ).text();
      hasil.thumb = $(
        "#venkonten > div.vezone > div.venser > div.post-thumb > img"
      ).attr("src");
      hasil.synopsis = $(
        "#venkonten > div.vezone > div.venser > div.venutama > div.lexot > p:nth-child(3)"
      ).text();
      hasil.dl_url_360p = $(
        "#venkonten > div.vezone > div.venser > div.venutama > div.lexot > div.dlbod > div > div:nth-child(2) > a:nth-child(2)"
      ).attr("href");
      hasil.dl_url_480p = $(
        "#venkonten > div.vezone > div.venser > div.venutama > div.lexot > div.dlbod > div > div:nth-child(3) > a:nth-child(2)"
      ).attr("href");
  ...
  return new Promise(async (resolve, reject) => {
    await axios
      ...
      .then(async (respon) => {
        ...
        await axios
          .get("https://igs.sf-converter.com/api/stories/" + id)
          .then((res) => {
...
function otakudesuongoing() {
  return new Promise((resolve, reject) => {
    axios
      .get(`https://otakudesu.bid`)
      .then(({ data }) => {
        const hasil = [];
        const $ = cheerio.load(data);
        $(
          "#venkonten > div > div.venser > div.venutama > div > div.rapi > div > ul > li"
        ).each(function (a, b) {
          result = {
            status: 200,
            author: author,
...
function komikindosearch(query) {
  return new Promise((resolve, reject) => {
    axios
      .get(`https://komikindo.id/?s=${query}`)
      .then(({ data }) => {
        const $ = cheerio.load(data);
        const hasil = [];
...
function nekopoi(url) {
  return new Promise((resolve, reject) => {
    ...
    axios.get(url).then((res) => {
      const $ = cheerio.load(res.data);
      hasil.thumb = $("#content > div.animeinfos > div.imgdesc > img").attr(
        "src"
      );
      hasil.synopsis = $(
        "#content > div.animeinfos > div.imgdesc > span > p"
      ).text();
      hasil.visitor_count = $(
        "#content > div.animeinfos > div.tabulasi > div:nth-child(3)"
      ).text();
      hasil.judul_jp = $(
        "#content > div.animeinfos > div.listinfo > ul > li:nth-child(1)"
      ).text();
      hasil.type = $(
        "#content > div.animeinfos > div.listinfo > ul > li:nth-child(2)"
      ).text();
      hasil.jmlh_epsd = $(
        "#content > div.animeinfos > div.listinfo > ul > li:nth-child(3)"
      ).text();
      hasil.status = $(
        "#content > div.animeinfos > div.listinfo > ul > li:nth-child(4)"
      ).text();
      hasil.publish = $(
        "#content > div.animeinfos > div.listinfo > ul > li:nth-child(5)"
      ).text();
      hasil.judul = $(
        "#content > div.animeinfos > div.listinfo > ul > li:nth-child(6)"
      ).text();
      hasil.genre = $(
        "#content > div.animeinfos > div.listinfo > ul > li:nth-child(7)"
      ).text();
      hasil.duration = $(
        "#content > div.animeinfos > div.listinfo > ul > li:nth-child(8)"
      ).text();
      hasil.rating = $(
        "#content > div.animeinfos > div.listinfo > ul > li:nth-child(9)"
      ).text();
      hasil.episode_url = [];
    });
    axios.get(url).then(({ data }) => {
      const $ = cheerio.load(data);
      $("#content > div.animeinfos > div.episodelist > ul > li").each(function (
        a,
        b
      ) {
        result = {
  ...
  return new Promise((resolve, reject) => {
    ...
    axios.get(url).then((res) => {
      ...
      hasil.studio = $(
        ...
        .split(": ")[1];
      hasil.genre = $(
        "#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(11)"
      )
        .text()
        .split(": ")[1];
      (hasil.thumbnail = $(
            ...
            data: new URLSearchParams(
              JSON.parse(
                gdata[1]
                  ...
                  .replace(/\]/g, "")
                  .replace(/text/g, "text[]")
                  .replace(/text\[\]pro\.me/g, "textpro.me")
              )
            ),
            headers: {
              cookie:
