...
export default async function (m, conn = { user: {} }) {
  ...
  if (m.sender) {
    ...
    console.log(
      `   ${chalk.cyan("- Message ID")}: ${m.msg?.id || m.key.id || "N/A"}`
    );
    console.log(
      `   ${chalk.cyan("- Sent Time")}: ${formatTime(m.messageTimestamp)}`
    );
    console.log(
      `   ${chalk.cyan("- Message Size")}: ${formatSize(filesize || 0)}`
    );
    console.log(`   ${chalk.cyan("- Sender ID")}: ${m.sender.split("@")[0]}`);
    console.log(
      `   ${chalk.cyan("- Sender Name")}: ${m.name ? conn.user.name : "N/A"}`
    );
  ...
  if (typeof m?.text === "string" && m.text) {
    ...
    const truncatedLog =
      logMessage.length > maxLogLength
        ? `${logMessage.slice(0, maxLogLength / 2)}...${logMessage.slice(
            -maxLogLength / 2
          )}`
        : logMessage;
    console.log(
      m.error != null
        ? `🚨 ${chalk.red(truncatedLog)}`
        : m.isCommand
        ? `⚙️ ${chalk.yellow(truncatedLog)}`
        : truncatedLog
    );
    console.log(chalk.cyan("╰──────────────────────────────────────···"));
  }
  ...
  if (m.msg) {
    ...
    if (/document/i.test(attachmentType)) {
      ...
      console.log(chalk.cyan("╭──────────────────────────────────────···"));
      console.log(
        chalk.redBright(`👨 Attached Contact: ${m.msg.displayName || "N/A"}`)
      );
      console.log(chalk.cyan("╰──────────────────────────────────────···"));
  ...
  if (m.sender) {
    ...
    console.log(
      chalk.blueBright(
        `  ${chalk.red("To")}: ${getPhoneNumber(conn.user?.jid)}`
      )
    );
...
const getContactInfo = (jid) =>
  `   - ${getPhoneNumber(jid)} ${
    conn.getName(jid) ? `~ ${conn.getName(jid)}` : ""
  }`;
  ...
  if (!codeUpdated) {
    ...
    readFile(file, "utf8", (err, data) => {
      ...
      writeFile(file, data, (writeErr) => {
        if (writeErr) {
          console.error(
            chalk.redBright("Error saving the updated code:"),
            writeErr
          );
        } else {
          codeUpdated = true;
          console.log(
            chalk.greenBright("Updated code has been saved to the file.")
          );
        }
      });
    });
