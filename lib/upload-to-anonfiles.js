...
async function download(fileURL, path) {
  if (
    !/((http|https):\/\/)(www.)?anonfiles\.com\b([-a-zA-Z0-9@:%._\+~#?&//=]*)/.test(
      fileURL
    )
  ) {
...
function extractRawURL(websiteData) {
  return websiteData.match(
    /https:\/\/cdn-[0-9]{3}.anonfiles.com\/[aA-zZ0-9]+\/[aA-zZ0-9]+-[aA-zZ0-9]+\/[^"]+/
  )[0];
}

function extractFileName(websiteData) {
  return websiteData
    .match(/text-center text-wordwrap">[^<]+/)[0]
    .replace('text-center text-wordwrap">', "");
}
